...
check_database(@)
{
    ...
    foreach my $DB ( @default_databases ) {
        if ( ! -f "$Perceus_Include::database/$DB.bdb" ) {
            &vprint("Creating new database: $DB");
            tie(%tmp, "DB_File", "$Perceus_Include::database/$DB.bdb", O_CREAT, 0644, $DB_BTREE) or
                &eprint("Could not create Berkeley BTree Database '$DB.bdb': $!\n");
            %tmp = ();
            untie %tmp;
        }
    }
...
list_databases(@)
{
    ...
    foreach my $file ( glob("$Perceus_Include::database/*.bdb") ) {
...
opendb(@)
{
    ...
    $self->{obj} = tie(%{$self->{db}}, "DB_File", "$Perceus_Include::database/$DB.bdb", $FLAGS, 0644, $DB_BTREE) or
        &perceus_die("Could not attach to Berkeley BTree Database '$DB.bdb': $!\n");
