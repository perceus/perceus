...
if [ "x$MIRROR" = "x" ]; then
    ...
    MIRROR=http://mirror.centos.org/centos-5
fi
...
cat <<EOF >$VNFSROOT/$NAME/etc/yum.repos.d/vnfs-centos5-base.repo
[vnfs-centos5-base]
name=CentOS-$releasever - Base
baseurl=$MIRROR/$releasever/os/$ARCH
        $VAULTMIRROR/$releasever/os/$ARCH
gpgkey=http://mirror.centos.org/centos/5/os/i386/RPM-GPG-KEY-CentOS-5
EOF
