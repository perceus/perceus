...
if [ "x$MIRROR" = "x" ]; then
    #MIRROR=http://ftp.scientificlinux.org/linux/scientific/45/
    MIRROR=http://mirror.mcs.anl.gov/pub/scientific-linux/45/
fi
...
cat <<EOF >$VNFSROOT/$NAME/etc/yum.repos.d/vnfs-scientific-linux4-base.repo
[vnfs-scientific-linux4-base]
name=Scientific Linux $releasever - Base
baseurl=$MIRROR/$ARCH/SL/RPMS/
gpgcheck=0
EOF
...
rm -rf $VNFSROOT/$NAME/etc/yum.repos.d/vnfs-scientific-linux4-base.repo
