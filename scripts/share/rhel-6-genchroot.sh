...
releasever=`rpm -q --qf '%{version}' --file /etc/redhat-release`
export releasever
...
yum -y -t -d 2 --releasever=$releasever --installroot $VNFSROOT/$NAME install \
   SysVinit basesystem bash redhat-release chkconfig coreutils cracklib \
   cracklib-dicts db4 e2fsprogs ethtool filesystem findutils gawk \
   glibc glibc-common grep initscripts iproute iputils krb5-libs \
   libacl libattr libgcc libstdc++ mingetty mktemp ncurses \
   net-tools nfs-utils pam pcre popt portmap procps psmisc rdate rsh \
   rsh-server rsync sed setup shadow-utils rsyslog tcp_wrappers \
   tzdata util-linux words xinetd zlib tar dracut less gzip which \
...
echo
echo "   # ${DIRNAME}/chroot2stateless.sh $VNFSROOT/$NAME /root/rhel-$releasever-1.stateless.$ARCH.vnfs"
