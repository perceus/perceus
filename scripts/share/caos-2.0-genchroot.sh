...
if [ "x$MIRROR" = "x" ]; then
    # use the LBL mirror
    MIRROR=http://altruistic.lbl.gov/mirrors/caosity/cAos-$releasever
fi
...
cat <<EOF >$VNFSROOT/$NAME/etc/yum.repos.d/vnfs-caos-core.repo
[vnfs-caos-core]
name=cAos2 core system repository
baseurl=$MIRROR/core/\$basearch
EOF

cat <<EOF >$VNFSROOT/$NAME/etc/yum.repos.d/vnfs-caos-ext-current.repo
[vnfs-caos-ext-current]
name=cAos2 extended current system repository
baseurl=$MIRROR/ext/current/\$basearch
EOF
...
yum -y -t -d 2 -c $VNFSROOT/$NAME/etc/yum.conf --installroot $VNFSROOT/$NAME install \
   SysVinit basesystem bash caos-release chkconfig coreutils cracklib \
   cracklib-dicts dev-minimal db4 e2fsprogs ethtool filesystem findutils gawk \
   ...
   util-linux module-init-tools udev linux-smp openssh-clients \
   openssh-server passwd dhclient pciutils vim-minimal shadow-utils \
   vixie-cron ntp strace grub detect
...
if ! grep -q "provisiond" $VNFSROOT/$NAME/etc/rc.local; then
cat <<EOF >> $VNFSROOT/$NAME/etc/rc.local
# Check to see if provisiond is installed, and if so run it
...
if [ "x\$MASTERIP" != "x" ]; then
   if [ -x "/usr/sbin/provisiond" ]; then
      /usr/sbin/provisiond -i 300 \${MASTERIP} ready \$NODEID
   elif [ -x "/sbin/provisiond" ]; then
      /sbin/provisiond -i 300 \${MASTERIP} ready \$NODEID
   elif [ -x "/sbin/provisiond-1" ]; then
      /sbin/provisiond-1 -i 300 \${MASTERIP} ready \$NODEID
   fi
fi
EOF
fi
...
rm -rf $VNFSROOT/$NAME/etc/yum.repos.d/vnfs-*.repo
...
echo
echo "   # ${DIRNAME}/chroot2stateless.sh $VNFSROOT/$NAME /root/caos-$releasever-1.stateless.$ARCH.vnfs"
