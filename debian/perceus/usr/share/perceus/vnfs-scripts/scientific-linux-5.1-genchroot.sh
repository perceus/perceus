...
if [ "x$MIRROR" = "x" ]; then
    #MIRROR=http://ftp.scientificlinux.org/linux/scientific/50/
    MIRROR=http://mirror.mcs.anl.gov/pub/scientific-linux/50/
fi
...
if [ "x$1" != "x" ]; then
   ...
   NAME="scientific-linux-$releasever-1.$ARCH"
fi
...
cat <<EOF >$VNFSROOT/$NAME/etc/yum.repos.d/vnfs-scientific-linux5-base.repo
[vnfs-scientific-linux5-base]
name=Scientific Linux $releasever - Base
baseurl=$MIRROR/$ARCH
gpgcheck=0
EOF
...
yum -y -t -d 2 -c $VNFSROOT/$NAME/etc/yum.conf --installroot $VNFSROOT/$NAME install \
   SysVinit basesystem bash sl-release chkconfig coreutils cracklib \
   ...
   util-linux module-init-tools udev kernel openssh-clients \
...
if [ -x "$VNFSROOT/$NAME/usr/sbin/pwconv" ]; then
   /usr/sbin/chroot $VNFSROOT/$NAME /usr/sbin/pwconv >/dev/null 2>&1||:
fi
if [ -x "$VNFSROOT/$NAME/sbin/chkconfig" ]; then
   /usr/sbin/chroot $VNFSROOT/$NAME /sbin/chkconfig xinetd on >/dev/null 2>&1
fi
...
cat <<EOF > $VNFSROOT/$NAME/etc/sysconfig/network-scripts/ifcfg-eth0
DEVICE=eth0
ONBOOT=yes
BOOTPROTO=dhcp
EOF

cat <<EOF > $VNFSROOT/$NAME/etc/sysconfig/network-scripts/ifcfg-eth1
DEVICE=eth1
ONBOOT=yes
BOOTPROTO=dhcp
EOF

/usr/sbin/chroot $VNFSROOT/$NAME umount /proc	>/dev/null 2>&1
rm -rf $VNFSROOT/$NAME/var/cache/yum/*
rm -rf $VNFSROOT/$NAME/var/log/*
rm -rf $VNFSROOT/$NAME/etc/yum.repos.d/vnfs-scientific-linux5-base.repo
...
echo
echo "   # ${DIRNAME}/chroot2stateless.sh $VNFSROOT/$NAME /root/scientific-linux-$releasever-1.stateless.$ARCH.vnfs"
